	...
	const bool debug)
{
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		fullprojmatrix.contiguous().data_ptr<float>(),
		campos.contiguous().data_ptr<float>(),
		deblur_nu.data_ptr<float>(),
		deblur_omega.data_ptr<float>(),
		deblur_rs_time,
		deblur_mb_time,
		deblur_samples,
		tan_fovx,
  ...
  return std::make_tuple(rendered, out_color, radii, out_depth, out_opaticy, n_touched, geomBuffer, binningBuffer, imgBuffer);
}
	...
	const bool debug) 
{
  ...
  torch::Tensor dL_dpixvels = torch::zeros({P, 2}, means3D.options());
  torch::Tensor dL_dtau = torch::zeros({P,6}, means3D.options());
  torch::Tensor dL_dnu = torch::zeros({3}, means3D.options());
  torch::Tensor dL_domega = torch::zeros({3}, means3D.options());
  ...
  {  
	  ...
	  dL_dpixvels.contiguous().data_ptr<float>(),
	  ...
	  dL_dnu.contiguous().data_ptr<float>(),
	  dL_domega.contiguous().data_ptr<float>(),
	  debug);
  }

  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations, dL_dtau, dL_dnu, dL_domega);
}
